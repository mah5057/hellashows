	...
	foreach ($result as $row)
    {
		...
		if($row[9]!=''){
			if(strpos($row[9],'/') == true){
				...
				array_push($showAttributes,'DOORS AT '.strtoupper($showTimes[0]).'</li><li>SHOW AT '.strtoupper($showTimes[1]));
			}
		...
		if($row[3]!=''){
			if($row[3] == 'free'){array_push($showAttributes,'FREE');}
				...
				array_push($showAttributes,$prices[0].' PRE-SALE</li><li>'.$prices[1].' AT THE DOOR');
			}					
